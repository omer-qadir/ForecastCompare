...
def voll_station_data():
    ...
    from forecast_db_interface import VollTable, forecast_db_interface, toFloat
    ...
    response = client._url_to_xml_tree(
        "http://eklima.met.no/metdata/MetDataService?invoke=getMetData&timeserietypeID=0&format=&from=&to=&stations=68860&elements=UM%2CPRM%2CRR%2CTAMRR%2CFFM%2CTAN%2CTAX%2CDD18%2CNNM&hours=&months=&username=",
        False,
        False,
    )
    ...
    for node in weatherElement[0].getElementsByTagName("item"):
        ...
        lutObservedVals[lutMetElements[currentId]] = node.getElementsByTagName("value")[
            0
        ].firstChild.data
